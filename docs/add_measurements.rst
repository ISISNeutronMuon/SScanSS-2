...
* A fixed reference plane can be used to mark a line of interest in the cross-section. To add reference plane, move
  the cross-sectional plane to a desired location using the slider or textbox then click the **Add Plane** button in the
  reference planes, a fixed plane will be added in the same location as the cross-section, the cross-section plane can
  then be adjusted to add more reference planes or to add measurement points. The line of intersection between the
  reference plane(s) and cross-section plane will be drawn along with the sample. All reference planes can be removed
  by clicking the **Remove Planes** button.
