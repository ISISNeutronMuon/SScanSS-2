...
def create_cuboid(width=1.0, height=1.0, depth=1.0):
    ...
    return Mesh(
        np.array(vertices, dtype=np.float32), np.array(indices, dtype=np.uint32), np.array(normals, dtype=np.float32)
    )
...
def create_tube(inner_radius=0.5, outer_radius=1.0, height=1.0, slices=64, stacks=64):
    ...
    vertices = np.vstack(
        (v_1, v_2, v_1[:slices, :], v_2[:slices, :], v_1[-slices:, :], v_2[-slices:, :])  # vertices for top face
    ...
    normals = np.vstack(
        (
            ...
            np.tile([0.0, -1.0, 0.0], (slices * 2, 1)),
        )
...
def create_plane(plane, width=1.0, height=1.0, slices=1, stacks=1):
    ...
    matrix = rotation_btw_vectors(plane.normal, np.array([0.0, 0.0, 1.0]))
