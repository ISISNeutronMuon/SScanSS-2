...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def showAlignmentError(self, indices, enabled, points, transform_result, end_configuration, order_fix=None):
        ...
        """
        alignment_error = AlignmentErrorDialog(
            self, indices, enabled, points, transform_result, end_configuration, order_fix
        )
    ...
    def showScriptExport(self):
        ...
        if simulation.isRunning():
            self.showMessage(
                "Finish or Stop the current simulation before attempting to write script.", MessageSeverity.Information
            )
            return
    ...
    def openProject(self, filename=""):
        ...
        if not filename:
            filename = self.showOpenDialog(
                "hdf5 File (*.h5)", title="Open Project", current_dir=self.presenter.model.save_path
            )
            if not filename:
                return

        self.presenter.useWorker(
            self.presenter.openProject, [filename], self.presenter.updateView, self.presenter.projectOpenError
        )
    ...
    def sceneSizeErrorHandler(self):
        ...
        msg = (
            f"The scene is too big the distance from the origin exceeds {self.gl_widget.scene.max_extent}mm."
...
class Updater:
    ...
    def check(self, startup=False):
        ...
        self.parent.presenter.useWorker(
            self.checkHelper, [], self.onSuccess, self.onFailure, self.parent.progress_dialog.close
        )
